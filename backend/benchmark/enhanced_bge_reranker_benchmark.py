"""
Enhanced BGE Reranker Benchmark: Fixed Relevance Calculation
==========================================================
...
Key Fixes:
- Corrected relevance calculation to find relevant documents across all samples
- Enhanced evaluation methodology for more realistic assessment
- Better document cross-matching for improved evaluation
...
def setup_logging():
    ...
    file_handler = logging.FileHandler(
        f'enhanced_bge_reranker_benchmark_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log', 
        encoding='utf-8'
    )
...
class EnhancedBGERerankerBenchmark:
    """Enhanced comprehensive benchmark for BGE reranker evaluation with fixed relevance calculation"""
    ...
    def setup(self):
        ...
        logger.info("Setting up Enhanced BGE Reranker Benchmark...")
        ...
        try:
            ...
            try:
                ...
                self.collection = self.chroma_client.create_collection(
                    ...
                    metadata={"description": "Enhanced BGE Reranker Benchmark Collection"}
                )
    ...
    def load_samples(self) -> List[Dict]:
        """Load samples from HotpotQA dataset with enhanced processing"""
    ...
    def index_documents(self, samples: List[Dict]):
        """Index documents in ChromaDB with enhanced relevance tracking"""
    ...
    def query_with_reranking(self, question: str, top_k: int = 20, initial_k: int = 50) -> Tuple[List[Dict], float, float]:
        ...
        if documents and self.bge_reranker:
            ...
            # Rerank documents
            reranked_indices = self.bge_reranker.rerank(question, documents)
            ...
            for idx in reranked_indices[:top_k]:
                reordered_documents.append({
                    ...
                    'rerank_score': len(reranked_indices) - reranked_indices.index(idx)  # Higher is better
                })
            
            # Log reranking insights
            if reordered_documents:
                logger.info(f"Top reranked doc: {reordered_documents[0]['metadata']['title'][:50]}...")
    ...
    def run_benchmark(self):
        ...
        logger.info(f"Running Enhanced BGE reranker benchmark on {self.sample_size} samples...")
    ...
    def analyze_results(self):
        ...
        for metric in ['ndcg@5', 'mrr', 'map', 'precision@5']:
            if metric in summary:
                improvement = summary[metric]['improvement_percent']
                logger.info(f"  {metric}: {improvement:+.2f}%")
        
        logger.info(f"⏱️  Average time overhead: {summary.get('total_time', {}).get('improvement_percent', 0):+.2f}%")
    ...
    def save_results(self):
        ...
        results_file = f"enhanced_bge_reranker_benchmark_results_{timestamp}.json"
        with open(results_file, 'w', encoding='utf-8') as f:
            json.dump(self.results, f, indent=2, ensure_ascii=False)
        logger.info(f"📄 Detailed results saved to: {results_file}")
        
        # Save summary statistics
        summary_file = f"enhanced_bge_reranker_benchmark_summary_{timestamp}.json"
        ...
        csv_file = f"enhanced_bge_reranker_benchmark_comparison_{timestamp}.csv"
    ...
    def create_visualizations(self):
        ...
        try:
            ...
            metrics_to_plot = ['ndcg@5', 'mrr', 'map', 'precision@5', 'recall@5']
            ...
            if improvements:
                ...
                plt.title('Enhanced BGE Reranker Performance Improvements', fontsize=16, fontweight='bold')
                ...
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                plot_file = f"enhanced_bge_reranker_benchmark_comparison_{timestamp}.png"
...
def main():
    ...
    logger.info("🚀 Starting Enhanced BGE Reranker Benchmark")
    logger.info("=" * 60)
    ...
    benchmark = EnhancedBGERerankerBenchmark(config)
    
    try:
        if benchmark.setup():
            benchmark.run_benchmark()
            logger.info("🎉 Enhanced BGE Reranker Benchmark completed successfully!")
        else:
